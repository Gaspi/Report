...
\item $X\cl{[(} x \cl{) := } y \cl{]}$
\item $X\cl{[(} x \cl{) <- } y \cl{]}$
\item $\cl{lambda(}  f_{ind} \cl{, } m \cl{, } x_1 \cl{, } ... \cl{, } x_m \cl{)}$ \ \ \ where $f_{ind} \in \N$ is a "function pointer".
...
\item $<\cl{if (} x \cl{) } a \cl{ else } b, (s,h) >  \ \longrightarrow \ 
\left\lbrace \begin{array}{ll}
< \cl{pop(}a\cl{)} , ( [] :: s, h) > & \text{if } s(x) = 0 \\
< \cl{pop(}b\cl{)} , ( [] :: s, h)> & \text{otherwise}
\end{array} \right. $
...
\end{eqnarray*}
\item $< \cl{lambda(} f_{ind} \cl{, } m \cl{, } x_1 \cl{, } ... \cl{, } x_m \cl{)} , S > \ \longrightarrow \ < ref(m), (s, h') > $ where
\begin{eqnarray*}
h(ref(m)) & & \text{ is not defined  \ \ \ \ ($ref(m)$ is a fresh pointer)} \\
...
\end{eqnarray*}
\item $< y\cl{(} x_1 \cl{, } ... \cl{, } x_n \cl{)}, (s,h) > \ \longrightarrow \ < \cl{pop(} [f] \cl{)}, ( (s' :: s), h) >$ where
\begin{eqnarray*}
E & := & h( s(y) ) \\
...
\item $<f\cl{(} x_1, ... , x_n \cl{)} , (s,h) > \ \longrightarrow \ < \cl{pop( } [f] \cl{)}, (s' :: s, h) > $ where
