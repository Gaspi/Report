...
If $f$ is a function declared in a program, we call $f_i$ its arguments (variables) and $[f]$ its body (expression).
\begin{prooftree}
\AxiomC{$<f\cl{(} x_i \cl{)} , s >  \ \Longrightarrow \  < [f], \uplus_{i = 0}^{n-1} (f_i \mapsto s(x_i) )$}
\UnaryInfC{$ <f\cl{(} x_i \cl{)} , s > \ \Longrightarrow \ v $}
\end{prooftree}

\begin{prooftree}
\AxiomC{$<y,s> \ \Longrightarrow \ n$}
\UnaryInfC{$<x \cl{[} y \cl{]} , s> \ \Longrightarrow \ s( s(x) + n)$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$<x,s> \ \Longrightarrow \ n > 0$}
\UnaryInfC{$<\cl{if } x \cl{ \char'173 \ } a \cl{ \char'175 \ else \char'173 \ } b \cl{ \char'175} , s> \ \Longrightarrow \ <a, s>$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$<I,s> \ \longrightarrow \ s'$}
\UnaryInfC{$<I;e , s> \ \Longrightarrow \ <e, s'>$}
\end{prooftree}

\begin{prooftree}
\AxiomC{}
\UnaryInfC{$<\cl{set(} x \cl{, } I \cl{; } e \cl{)} , s> \ \longrightarrow \ <I \cl{; } \cl{set(} x \cl{, } e \cl{)}, s >$}
\end{prooftree}

\begin{prooftree}
\AxiomC{}
\UnaryInfC{$<\cl{set(} x \cl{, } \cl{if } y \cl{ \char'173 \ } a \cl{ \char'175 \ else \char'173 \ } b \cl{ \char'175 \ )} , s> \ \longrightarrow \
< \cl{if } y \cl{ \char'173 \ set(} x \cl{ , } a \cl{)  \char'175 \ else \char'173 \ set(} x \cl{ , } b \cl{) \char'175} , s >$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$<e , s> \ \Longrightarrow \ v$}
\UnaryInfC{$<\cl{set(} x \cl{, } e \cl{)} , s> \ \longrightarrow \ s \uplus (x \mapsto v)$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\forall i \in \{0, ..., n-1\}, \ \ \ < e , s \uplus ( x \mapsto i) > \ \Longrightarrow \ v_i $}
\UnaryInfC{$ <\cl{init\_array(} X \cl{, } x \cl{, } e \cl{)} , s > \ \longrightarrow \ s \uplus_{i = 0}^{n-1} ( s(X) + i \mapsto v_i) $}
\end{prooftree}

\begin{prooftree}
\AxiomC{}
\UnaryInfC{$<\cl{update(} X \cl{, } Y \cl{, } x \cl{, } y \cl{)} , s> \ \longrightarrow \ s \uplus_{i \neq s(x)} ( s(X) + i \mapsto s(s(Y) + i) ) \uplus ( s(X) + s(x) \mapsto s(y))$}
\end{prooftree}

\begin{prooftree}
\AxiomC{}
\UnaryInfC{$ <\cl{destr\_update(} X \cl{, } x \cl{, } y \cl{)} , s> \ \longrightarrow \ s \uplus ( s(X) + s(x) \mapsto s(y)) $}
\end{prooftree}
...
LV\left( I \cl{; } e \right) & := & LV(I) \cup LV(e) \\
LV\left( \cl{set(} x \cl{, } e \cl{)} \right) & := & LV(e) \\
LV\left( \cl{init\_array(} X \cl{, } x \cl{, } e \cl{)} \right) & := & LV(e) - \{ x \} \\
LV\left( \cl{update(} X \cl{, } Y \cl{, } x \cl{, } y \cl{)} \right) & := & \{ Y, x, y \} \\
LV\left( \cl{destructive\_update(} X \cl{, } x \cl{, } y \cl{)} \right) & := & \{ x, y \}
\end{eqnarray*}


The translator from PVS to that intermediate language guarantees the following properties 
\begin{itemize}
\item All arguments passed to a function are \testbf{different} variables.
