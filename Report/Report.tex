...
Each PVS function is translated into two different C functions: a safe non destructive version and a destructive version.

The destructive version of a function may have some requirements on its arguments. Its arguments may be flagged as \mut, meaning that every call to this version of the function must make sure that the arguments passed can be safely destructively updated.
...
\item Only expressions can be flagged \dupl.
...
\item A variable is flagged bang iif it is created with a \cl{copy}, \cl{init\_array}, \cl{init\_record} or is the result of a call to a function flagged \bang.
...
\end{itemize}
\item If a single occurence of an argument is flagged \dupl, then the argument is flagged \dupl.
\end{itemize}
...
flags : \bang (variable), \safe (occurences), \dupl (variable)
...
\subsubsection{\texttt{if} expressions}
Represented by \texttt{if-expr}

\subsubsection{Integer, rationnals}
