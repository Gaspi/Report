...
Whereas PVS syntax relies on expressions, the C language relies on memory allocation and on the use of variables. Even though some simple PVS expression (for instance \cl{2*(x+y)} for \cl{x} and \cl{y} typed as small integers) can be translated to C expressions (\cl{2*(x+y)} with \cl{x} and \cl{y} typed as \cl{int}), most of the time to represent a PVS expression in the C language, we need to build it and store it into a variable. For instance \cl{X[0] + 1} should be translated to
\begin{lstlisting}
mpz_t aux = X[0];
mpz_t one;
...
\begin{lstlisting}
  mpz_t* A = malloc( 10 * sizeof(mpz_t) );
  ...
  mpz_clear(R);
  [...] // use A here
  for(i=0; i<10; i++)  mpz_clear(A[i]);
  free(A);
\end{lstlisting}

\begin{tabular}{|l|p{5.2cm}|p{4cm}|p{1cm}|p{2cm}|}
\hline
To compute & Needed & I & E & D  \\ \hline
$ R(E, \cl{mpz\_t*})$ &
...
\begin{itemize}
\item $R$ which take an expression and a type and may add an extra conversion in the instructions to make sure its result has the expected type. Also the result of this function has a proper name.
