...
\item Induction step:
\begin{itemize}
\item References defined in the store are never undefined. If $<e, S> \ \longrightarrow \ <e', S'>$ and $h$ defined on $r$ then $h$ defined on $r'$. This is easily proven since no redex remove a definition in the heap.
\item Whenever a value in the heap or in the store is modified, it is replaced with functions (redex 7), integers (redex 4 and 7), values from the heap (redexes 2, 5, 6 and 8), values from the stack (redexes 1, 5, 6, 7, 8, 9, 10 and 11) or values reduced from an expression (redexes 12 and 13).
\end{itemize}
\end{enumerate}
\end{proof}
...
We introduce \emph{update contexts} as an expression with a single occurence of a hole:
\begin{enumerate}
\itemsep-0.2em
\item $\ucont{}$
...
The variables that may be aliased to a variable $X$, in a context $U$ is defined as 
\begin{eqnarray*}
Lv(\ucont{} ) &:=& \emptyset \\
Lv( \cl{set(} x \cl{, } U \cl{); } e ) &:=& Lv(U) \cup ( Fv(e) - \{ x \} ) \\
Lv( \cl{set(} x \cl{, } a \cl{); } U ) &:=& Lv(U) \cup \left\lbrace \begin{array}{ll}
Av(a) & \text{if $x \in Lv(U)$.} \\
\emptyset & \text{otherwise.}
\end{array} \right. \\
Lv( \cl{if (} x \cl{) } U \cl{ else } b ) &:=& Lv(U) \\
Lv( \cl{if (} x \cl{) } a \cl{ else } U ) &:=& Lv(U) 
\end{eqnarray*}

The variables live in a context are defined as the variables that are 
\begin{eqnarray*}
Lv(\ucont{} ) &:=& \emptyset \\
Lv( \cl{set(} x \cl{, } U \cl{); } e ) &:=& Lv(U) \cup ( Fv(e) - \{ x \} ) \\
Lv( \cl{set(} x \cl{, } a \cl{); } U ) &:=& Lv(U) \cup \left\lbrace \begin{array}{ll}
