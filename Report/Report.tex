...
$$ BA(g^d) \ni g^d_i \neq g^d_j \in BA(g^d) \Longrightarrow Cv(U)(x_i) \cap Cv(U)(x_j) = \emptyset$$
\end{itemize}
...
\begin{enumerate}
\item The function call to $f^d$ have the same evaluation. This means they can't be told apart by looking only at the value of the result. However they differ in the use of their arguments.
\item When the function call $f(x_i)$ is evaluated with a store $S$, the only references which value could be modified is $\bigcup_{f_i \in BA(f)} S(x_i) \cup \bigcup_{f^*_i \in BA(f)} Ac(S)(x_i)$.
...
\begin{itemize}
\item[$(H1)$] $\forall i, \ Cv(U)(x_i) \cap Lv(U) = \emptyset $ \ \ (Analysis)
\item[$(H2)$] $\forall i \neq h, \ Cv(U)(x_i) \cap Cv(U)(x_j) = \emptyset$ \ \ (Analysis)
\item[$(H3)$] The only references which value may be modified during the evaluation of the call with a store $S$ is the union $\bigcup S(x_i) \cup \bigcup Ac(S)(x^*_i)$.
\item[$(H4)$] The evaluation of the call to $g^d$ with a store $S$ is the same than if $g$ was called.
\item[$(H5)$] $f$ will always be called in an evaluation context with a store $S$ such that the sets $Ac(S)(f_i)$ are disjoint for all $f_i \in BA(f)$.
\end{itemize}
\begin{proposition}
...
\end{proof}
\begin{proposition}
For all store $S$ with which the function call is evaluated, the sets $\bigcup S(x_i) \cup \bigcup Ac(S)(x^*_i)$ and $Ac(S)(U)$ are disjoints.
\end{proposition}
\begin{proof}
$(H1)$ and $(H5)$.
\end{proof}
