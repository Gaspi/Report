...
\vspace{1.5in}
\begin{abstract}
PVS (standing for Prototype Verification System), is an Open Source project developped by CSL at SRI International and aiming to be both a semi-automated theorem prover providing formal support for conceptualization and debugging in the early stages of the design of hardware or software systems and a programming language. The evaluation of PVS expressions relies so far on a build-in PVS interpreter based on Common Lisp, called "Ground Evaluator". In order to allow the integration of PVS code as well as its fast execution for debugging and testing purposes, we describe here a translator of a subset of PVS to the language C.\\
...
However the execution of updates by copying is often redundant and could be safely implemented by means of destructive, inplace updates in an imperative program.
We describe a method for analyzing and replacing the safe updates in an imperative program by destructive, inplace update.
...
For $X$ a variable and $U$ a context, we also consider the set of critical variables $Cv(U)(X)$ smaller set such that :
$$\forall Y \ , \ \ PA(U)(Y) \subset Cv(U)(X) \ \text{ or } PA(U)(Y) \cap Cv(U)(X) = \emptyset $$
This set contain all variables that may point to $X$ and all variable that may point to these variables and so on. \\


The variables live in a context are the variables that could be evaluated after the hole the in the context and the variable that these variables may point to and so on.\\
...
The set $Lv(U)$ of the variables live in a context $U$ is defined as the smallest set containing $Lv^0(U)$
\begin{eqnarray*}
Lv^0(\ucont{} ) &:=& \emptyset \\
...
\subsection{Difference with the previous analysis of PVS}
